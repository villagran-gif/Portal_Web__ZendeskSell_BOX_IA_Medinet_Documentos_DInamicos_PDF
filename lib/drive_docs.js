...
/**
 * Ensure header exists and inserts a gray small text with deal + agent info.
 * Priority: agent name, then email, then phone, then only deal id.
 */
async function ensureDealAgentHeader({ documentId, dealId, agentName, agentEmail, agentPhone }) {
  const docs = getDocs();
  const safeDealId = String(dealId || '').trim();
  if (!safeDealId) return null;

  const safeName = String(agentName || '').replace(/\s+/g, ' ').trim();
  const safeEmail = String(agentEmail || '').replace(/\s+/g, ' ').trim();
  const safePhone = String(agentPhone || '').replace(/\s+/g, ' ').trim();

  let agentPart = '';
  if (safeName) {
    agentPart = `Agente: ${safeName}`;
  } else if (safeEmail) {
    agentPart = `Agente: ${safeEmail}`;
  } else if (safePhone) {
    agentPart = `Agente: ${safePhone}`;
  }

  const headerText = agentPart
    ? `DEAL.${safeDealId} · ${agentPart}
`
    : `DEAL.${safeDealId}
`;
